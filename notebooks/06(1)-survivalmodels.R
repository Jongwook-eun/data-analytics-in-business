...
#Surv function은 time duration("ChurnTime"),indicator("1-Censored") 먼저 정의해줘야함 
...
summary(surv.res) 
#survreg결과로 나온 coefficients(delta estimates)들은 의미있는게아니라서 a,beta로 transformation 해줘야함 
...
#Income variable의 beta계수가 0.0197이라는 말은, one unit of customer Income이 increase할수록 
#hazard rate(instantaneous probability density of repurchase occurring)이 1.97% 상승한다는의미
#또 Income의 delta계수(-0.0087)와 beta계수(0.0197)은 부호가 반대인데,
#이는 hazard rate(corresponding independent value)이 increase할수록 "expected duration before repurchase" 가 decrease한다는 의미

#plot the hazard function
xbar = colMeans(mydata[,2:10]) #calculate the mean x values (2,3,4는 three x variables of the data)
...
#a값은 2.255127 >> 1보다 크므로 positive duration dependence
#longer individual remains in the current state >>raise the probability it will switch into the other state
#∴hazard function실행시켜보면 time 커질수록 증가하는 형태

#plot the density function of duration distribution : dweibull function을 통해 Weibull distribution의 PDF도출가능
b=exp(-xbeta/a)
curve(dweibull(x, shape=a, scale=b),xlab="ChurnTime",ylab="density",xlim=c(0,20),ylim=c(0,0.12))
# a > 1이므로 hazard는 시간이 지날수록 증가하는 형태를 보이며
# PDF는 초기에는 증가하다가 어떤 시점 이후 감소하는 종 모양을 보임
# 이는 사건이 특정 시점에서 가장 많이 발생하는 경향을 보여주는 것임
# (실제 데이터에서는 right-censoring 때문에 tail 부분에서 급격히 작아질 수 있음)
